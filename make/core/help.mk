...
CY_HELP_TOOLCHAIN_VERBOSE=Supported toolchains for this target are, [ $(CY_SUPPORTED_TOOLCHAINS) ].\
							...
							CY_COMPILER_PATH="<path>/IAR Systems/Embedded Workbench 8.2/arm/bin"
...
CY_HELP_VERBOSE_VERBOSE=Setting VERBOSE to true may help in debuggging build errors/warnings. By default it is set to false.\
...
CY_HELP_SOURCES_VERBOSE=This can be used to include files external to the application directory.\
						The path can be both absolute or relative to the working directory.\
						$(CY_NEWLINE)$(CY_NEWLINE)Example Usage (within makefile): SOURCES+=path/to/file/Source1.c
CY_HELP_INCLUDES=Specifies include paths not under the working directory. The path can be both absolute or\
...
# BSP variables
#
CY_HELP_DEVICE=Device ID for the primary MCU on the target board/kit. This is generally set by the BSP.
CY_HELP_DEVICE_VERBOSE=The device identifier is mandatory for all board/kits.
CY_HELP_ADDITIONAL_DEVICES=IDs for additional devices on the target board/kit. This is generally set by the BSP if applicable.
...
CY_HELP_DEVICE_GEN_VERBOSE=This is an optional variable when calling the "bsp" make target to replace the primary\
					...
					$(CY_NEWLINE)$(CY_NEWLINE)Example Usage: make bsp TARGET_GEN=MyBSP DEVICE_GEN=CY8C624ABZI-D44
...
CY_HELP_CY_GETLIBS_DEPS_PATH_VERBOSE=Setting this path allows relocating the directory that the library-manager uses to store\
					...
					Usage is similar to to CY_GETLIBS_PATH.\
...
CY_HELP_CY_GETLIBS_CACHE_PATH_VERBOSE=The build system caches all cloned repos in a directory named /cache \
					...
					elsewhere on disk. Usage is similar to to CY_GETLIBS_PATH. To disable the cache entirely, \
...
CY_HELP_CY_GETLIBS_OFFLINE_PATH_VERBOSE=The offline content is used to create/update applications when not connected\
					...
					offline content to elsewhere on disk. Usage is similar to to CY_GETLIBS_PATH.\
					$(CY_NEWLINE)Note: This variable requires ModusToolbox tools_2.1 or higher.
