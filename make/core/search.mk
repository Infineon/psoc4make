...
CY_IGNORE_PRUNE:=-name ‘.git’ $(addprefix -o -path ,$(abspath $(filter-out $(CY_IGNORE_FILES),$(CY_IGNORE_DIRS))))
...
#
CY_SEARCH_ALL_FILES:=$(sort $(patsubst $(CY_SEARCH_APP_PATH)/%,$(CY_INTERNAL_APP_PATH)/%,\
                        $(patsubst $(CY_SEARCH_EXTAPP_PATH)/%,$(CY_INTERNAL_EXTAPP_PATH)/%,\
                        $(shell $(CY_FIND) -L $(CY_SEARCH_APP_PATH) $(CY_SEARCH_EXTAPP_PATH) $(CY_SEARCH_ASSET) $(CY_DEPAPP_EXTAPP_DIRS) \
...
CY_SEARCH_AVAILABLE_INCLUDES:=\
    ...
    $(sort $(foreach asset,$(CY_SEARCH_ASSET),$(call CY_MACRO_SEARCH_PARENT,$(CY_SEARCH_ASSET_INCLUDES),$(asset))))\
